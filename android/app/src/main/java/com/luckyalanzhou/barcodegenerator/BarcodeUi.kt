...
internal fun MainActivity.showLanShare() {
    ...
    content.addView(LinearLayout(this).apply {
        ...
        addView(ImageButton(this@showLanShare).apply { setImageResource(R.drawable.ic_qr_code); setColorFilter(0xff0a84ff.toInt()); setPadding(dp(14), dp(14), dp(14), dp(14)); setBackgroundColor(Color.TRANSPARENT); contentDescription = "显示二维码"; setOnClickListener {
...
internal fun MainActivity.uploadLanShareFile(uri: Uri, temporaryFile: File? = null) { val session = lanShareSession ?: return; lifecycleScope.launch(kotlinx.coroutines.Dispatchers.IO) { runCatching { val name = lanShareManager.upload(session, uri); name to lanShareManager.list(session) }.onSuccess { (name, files) -> temporaryFile?.delete(); runOnUiThread { files.firstOrNull { it.name == name }?.let { lanShareOwnFileIds.add(it.id) }; lanShareFiles = files; render() } }.onFailure { temporaryFile?.delete(); runOnUiThread { toast("上传失败") } } } }
internal fun MainActivity.uploadLanShareMessage(text: String) { val session = lanShareSession ?: return; lifecycleScope.launch(kotlinx.coroutines.Dispatchers.IO) { runCatching { val name = lanShareManager.uploadText(session, text); name to lanShareManager.list(session) }.onSuccess { (name, files) -> runOnUiThread { lanShareMessageInput?.setText(""); files.firstOrNull { it.name == name }?.let { lanShareOwnFileIds.add(it.id) }; lanShareFiles = files; render(); toast("发送成功") } }.onFailure { runOnUiThread { toast("发送失败") } } } }
