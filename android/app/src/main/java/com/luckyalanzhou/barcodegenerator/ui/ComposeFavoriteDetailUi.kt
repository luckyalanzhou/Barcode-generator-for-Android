...
internal fun ComposeFavoriteDetailPage(activity: MainActivity, group: FavoriteGroup) {
    ...
    Column(Modifier.fillMaxWidth().padding(horizontal = 10.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Text("${currentGroup.folder} · 保存于 ${activity.formatSavedTime(currentGroup.savedAt)}", color = secondary, fontSize = 13.sp, textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth())
        OutlinedButton(
            ...
            groupItems.forEach { item ->
                Column(
                    Modifier.fillMaxWidth().background(card, RoundedCornerShape(16.dp)).combinedClickable(
                        onClick = {},
                        onLongClick = { activity.performHapticFeedback(); activity.openFavoriteForEditing(currentGroup) }
