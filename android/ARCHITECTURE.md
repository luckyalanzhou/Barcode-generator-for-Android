...
- 分享会话通过二维码或连接地址访问；服务端不进行身份验证或访问码校验，同一局域网内任何知道地址的设备都可读取、上传和下载共享文件。
- 传输使用未加密 HTTP；不要在不可信网络中分享敏感文件。结束后关闭分享服务，以停止网络访问。
- 文件大小、房间容量、文件名和上传流仍由服务端校验；App 加入会话时仍检查地址是否位于当前路由器子网内。这些限制不是访问认证，浏览器访问端口的其他设备仍可直接连接。
